...
# then log output of file /code/bin/manti
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/manti \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/manti | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/manti /usr/bin/manti
