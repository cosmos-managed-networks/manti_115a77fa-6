...
HTTPS_GIT := github.com/manti/manti.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=manti \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=manti \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/manti ./cmd/manti
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/manti.exe ./cmd/manti
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/manti
...
draw-deps:
	...
	@goviz -i ./cmd/manti -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c manti --local -f chains.yaml
endif
...
set-testnet-configs:
	manti config set client chain-id chainid-1
	manti config set client keyring-backend test
	manti config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | manti keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | manti keys add acc1 --recover`
